...
func UpdatewebhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/webhooks/%s", cfg.BaseURL, webhookId)
		...
		var result models.UpdateWebhookResult
...
func CreateUpdatewebhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_webhooks_webhookId",
		mcp.WithDescription(" Updates a webhook. "),
		...
		mcp.WithString("webhookId", mcp.Required(), mcp.Description(" The unique ID for a webhook. ")),
		mcp.WithString("branchName", mcp.Description("Input parameter:  The name for a branch that is part of an Amplify app. ")),
		mcp.WithString("description", mcp.Description("Input parameter:  The description for a webhook. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatewebhookHandler(cfg),
	}
}
