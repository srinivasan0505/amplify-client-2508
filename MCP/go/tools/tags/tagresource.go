...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_resourceArn",
		mcp.WithDescription(" Tags the resource with a tag key and value. "),
		...
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description(" The Amazon Resource Name (ARN) to use to tag a resource. ")),
		mcp.WithObject("tags", mcp.Required(), mcp.Description("Input parameter:  The tags used to tag the resource. ")),
	)
