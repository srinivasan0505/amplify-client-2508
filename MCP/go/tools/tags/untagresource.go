...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_tags_resourceArn#tagKeys",
		mcp.WithDescription(" Untags a resource with a specified Amazon Resource Name (ARN). "),
		...
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description(" The Amazon Resource Name (ARN) to use to untag a resource. ")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description(" The tag keys to use to untag a resource. ")),
	)
