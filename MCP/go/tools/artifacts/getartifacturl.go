...
func GetartifacturlHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/artifacts/%s", cfg.BaseURL, artifactId)
		...
		var result models.GetArtifactUrlResult
...
func CreateGetartifacturlTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_artifacts_artifactId",
		mcp.WithDescription(" Returns the artifact info that corresponds to an artifact id. "),
		...
		mcp.WithString("artifactId", mcp.Required(), mcp.Description(" The unique ID for an artifact. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetartifacturlHandler(cfg),
	}
}
