...
func CreatedomainassociationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateDomainAssociationrequest
		...
		url := fmt.Sprintf("%s/apps/%s/domains", cfg.BaseURL, appId)
		...
		var result models.CreateDomainAssociationResult
...
func CreateCreatedomainassociationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apps_appId_domains",
		mcp.WithDescription(" Creates a new domain association for an Amplify app. This action associates a custom domain with the Amplify app "),
		...
		mcp.WithArray("subDomainSettings", mcp.Required(), mcp.Description("Input parameter:  The setting for the subdomain. ")),
		mcp.WithArray("autoSubDomainCreationPatterns", mcp.Description("Input parameter:  Sets the branch patterns for automatic subdomain creation. ")),
		mcp.WithString("autoSubDomainIAMRole", mcp.Description("Input parameter:  The required AWS Identity and Access Management (IAM) service role for the Amazon Resource Name (ARN) for automatically creating subdomains. ")),
		mcp.WithString("domainName", mcp.Required(), mcp.Description("Input parameter:  The domain name for the domain association. ")),
		mcp.WithBoolean("enableAutoSubDomain", mcp.Description("Input parameter:  Enables the automated creation of subdomains for branches. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatedomainassociationHandler(cfg),
	}
}
