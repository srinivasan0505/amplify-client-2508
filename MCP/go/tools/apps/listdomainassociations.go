...
func ListdomainassociationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apps/%s/domains%s", cfg.BaseURL, appId, queryString)
		...
		var result models.ListDomainAssociationsResult
...
func CreateListdomainassociationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apps_appId_domains",
		mcp.WithDescription(" Returns the domain associations for an Amplify app. "),
		...
		mcp.WithString("nextToken", mcp.Description(" A pagination token. Set to null to start listing apps from the start. If non-null, a pagination token is returned in a result. Pass its value in here to list more projects. ")),
		mcp.WithNumber("maxResults", mcp.Description(" The maximum number of records to list in a single response. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListdomainassociationsHandler(cfg),
	}
}
