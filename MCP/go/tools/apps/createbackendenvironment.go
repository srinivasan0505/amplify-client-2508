...
func CreatebackendenvironmentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateBackendEnvironmentrequest
		...
		url := fmt.Sprintf("%s/apps/%s/backendenvironments", cfg.BaseURL, appId)
		...
		var result models.CreateBackendEnvironmentResult
...
func CreateCreatebackendenvironmentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apps_appId_backendenvironments",
		mcp.WithDescription(" Creates a new backend environment for an Amplify app. "),
		...
		mcp.WithString("deploymentArtifacts", mcp.Description("Input parameter:  The name of deployment artifacts. ")),
		mcp.WithString("environmentName", mcp.Required(), mcp.Description("Input parameter:  The name for the backend environment. ")),
		mcp.WithString("stackName", mcp.Description("Input parameter:  The AWS CloudFormation stack name of a backend environment. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatebackendenvironmentHandler(cfg),
	}
}
