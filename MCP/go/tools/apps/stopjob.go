...
func StopjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apps/%s/branches/%s/jobs/%s/stop", cfg.BaseURL, appId, branchName, jobId)
...
func CreateStopjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apps_appId_branches_branchName_jobs_jobId_stop",
		mcp.WithDescription(" Stops a job that is in progress for a branch of an Amplify app. "),
