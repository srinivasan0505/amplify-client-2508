...
func ListappsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apps%s", cfg.BaseURL, queryString)
		...
		var result models.ListAppsResult
...
func CreateListappsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apps",
		mcp.WithDescription(" Returns a list of the existing Amplify apps. "),
		...
		mcp.WithString("nextToken", mcp.Description(" A pagination token. If non-null, the pagination token is returned in a result. Pass its value in another request to retrieve more entries. ")),
		mcp.WithNumber("maxResults", mcp.Description(" The maximum number of records to list in a single response. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListappsHandler(cfg),
	}
}
