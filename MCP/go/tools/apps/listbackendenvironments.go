...
func ListbackendenvironmentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["environmentName"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("environmentName=%v", val))
		}
		...
		url := fmt.Sprintf("%s/apps/%s/backendenvironments%s", cfg.BaseURL, appId, queryString)
		...
		var result models.ListBackendEnvironmentsResult
...
func CreateListbackendenvironmentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apps_appId_backendenvironments",
		mcp.WithDescription(" Lists the backend environments for an Amplify app. "),
		...
		mcp.WithString("environmentName", mcp.Description(" The name of the backend environment ")),
		mcp.WithString("nextToken", mcp.Description(" A pagination token. Set to null to start listing backend environments from the start. If a non-null pagination token is returned in a result, pass its value in here to list more backend environments. ")),
		mcp.WithNumber("maxResults", mcp.Description(" The maximum number of records to list in a single response. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListbackendenvironmentsHandler(cfg),
	}
}
