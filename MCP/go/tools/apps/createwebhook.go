...
func CreatewebhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apps/%s/webhooks", cfg.BaseURL, appId)
		...
		var result models.CreateWebhookResult
...
func CreateCreatewebhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apps_appId_webhooks",
		mcp.WithDescription(" Creates a new webhook on an Amplify app. "),
		...
		mcp.WithString("branchName", mcp.Required(), mcp.Description("Input parameter:  The name for a branch that is part of an Amplify app. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatewebhookHandler(cfg),
	}
}
