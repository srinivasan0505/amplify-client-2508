...
func UpdateappHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateApprequest
		...
		url := fmt.Sprintf("%s/apps/%s", cfg.BaseURL, appId)
		...
		var result models.UpdateAppResult
...
func CreateUpdateappTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apps_appId",
		mcp.WithDescription(" Updates an existing Amplify app. "),
		...
		mcp.WithArray("autoBranchCreationPatterns", mcp.Description("Input parameter:  Describes the automated branch creation glob patterns for an Amplify app. ")),
		...
		mcp.WithBoolean("enableBranchAutoBuild", mcp.Description("Input parameter:  Enables branch auto-building for an Amplify app. ")),
		mcp.WithString("basicAuthCredentials", mcp.Description("Input parameter:  The basic authorization credentials for an Amplify app. You must base64-encode the authorization credentials and provide them in the format <code>user:password</code>.")),
		mcp.WithBoolean("enableBasicAuth", mcp.Description("Input parameter:  Enables basic authorization for an Amplify app. ")),
		mcp.WithString("repository", mcp.Description("Input parameter:  The name of the repository for an Amplify app ")),
		...
		mcp.WithString("name", mcp.Description("Input parameter:  The name for an Amplify app. ")),
		...
		mcp.WithObject("environmentVariables", mcp.Description("Input parameter:  The environment variables for an Amplify app. ")),
		...
		mcp.WithArray("customRules", mcp.Description("Input parameter:  The custom redirect and rewrite rules for an Amplify app. ")),
		mcp.WithString("description", mcp.Description("Input parameter:  The description for an Amplify app. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdateappHandler(cfg),
	}
}
