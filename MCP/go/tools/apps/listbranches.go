...
func ListbranchesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apps/%s/branches%s", cfg.BaseURL, appId, queryString)
		...
		var result models.ListBranchesResult
...
func CreateListbranchesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apps_appId_branches",
		mcp.WithDescription(" Lists the branches of an Amplify app. "),
		...
		mcp.WithString("nextToken", mcp.Description(" A pagination token. Set to null to start listing branches from the start. If a non-null pagination token is returned in a result, pass its value in here to list more branches. ")),
		mcp.WithNumber("maxResults", mcp.Description(" The maximum number of records to list in a single response. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListbranchesHandler(cfg),
	}
}
