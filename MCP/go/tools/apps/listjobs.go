...
func ListjobsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apps/%s/branches/%s/jobs%s", cfg.BaseURL, appId, branchName, queryString)
...
func CreateListjobsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apps_appId_branches_branchName_jobs",
		mcp.WithDescription(" Lists the jobs for a branch of an Amplify app. "),
		...
		mcp.WithString("branchName", mcp.Required(), mcp.Description(" The name for a branch. ")),
		mcp.WithString("nextToken", mcp.Description(" A pagination token. Set to null to start listing steps from the start. If a non-null pagination token is returned in a result, pass its value in here to list more steps. ")),
