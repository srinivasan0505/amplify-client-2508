...
func GetjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apps/%s/branches/%s/jobs/%s", cfg.BaseURL, appId, branchName, jobId)
		...
		var result models.GetJobResult
...
func CreateGetjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apps_appId_branches_branchName_jobs_jobId",
		mcp.WithDescription(" Returns a job for a branch of an Amplify app. "),
		...
		mcp.WithString("jobId", mcp.Required(), mcp.Description(" The unique ID for the job. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetjobHandler(cfg),
	}
}
